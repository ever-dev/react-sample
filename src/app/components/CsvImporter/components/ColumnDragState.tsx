...
export function useColumnDragState(
  ...
  const [fieldAssignments, setFieldAssignments] = useState<FieldAssignmentMap>(initialAssignments);
  ...
  const internalAssignHandler = useCallback((column: Column, fieldName: string | null) => {
  ...
  const bindDrag = useDrag(({ first, last, event, xy, args }) => {
    if (first && event) {
      ...
      setDragState({
        pointerStartInfo: {
          initialXY: [xy[0], xy[1] + window.scrollY],
          initialWidth:
            event.currentTarget instanceof HTMLElement ? event.currentTarget.offsetWidth : 0,
          initialHeight:
            event.currentTarget instanceof HTMLElement ? event.currentTarget.offsetHeight : 0,
        },
        column,
    ...
    if (dragState && dragState.updateListener) {
      dragState.updateListener([xy[0], xy[1] + window.scrollY]);
    }
  }, {});
