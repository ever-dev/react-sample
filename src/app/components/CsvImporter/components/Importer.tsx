...
const FieldDefinitionContext = React.createContext<((setter: FieldListSetter) => void) | null>(
  null,
);
...
export const ImporterField: React.FC<ImporterFieldProps> = ({ name, label, optional }) => {
...
export function Importer<Row extends BaseRow>({
  ...
  const [fieldAssignments, setFieldAssignments] = useState<FieldAssignmentMap | null>(null);
  ...
  if (selectedFile === null) {
    return (
      <ContentWrapper
        ...
        content={content}
      >
        <FileSelector onSelected={fileHandler} onClose={onClose} />
      </ContentWrapper>
    );
  }
