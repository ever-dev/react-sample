  ...
  return new Promise<PreviewResults>(resolve => {
    ...
    Papa.parse(nodeStream, {
      ...
      chunk: ({ data, errors }, parser) => {
        ...
        data.forEach(row => {
          rowAccumulator.push(
            (row as unknown[]).map(item => (typeof item === 'string' ? item : '')),
          );
        });
    ...
    Papa.parse(nodeStream, {
      ...
      chunk: ({ data }, parser) => {
        ...
        const rows = (skipped ? data.slice(1) : data).map(row => {
          const stringRow = (row as unknown[]).map(item => (typeof item === 'string' ? item : ''));

          // perform BOM skip on first value
          if (skipBOM && stringRow.length > 0) {
            skipBOM = false;
            stringRow[0] =
              stringRow[0].charCodeAt(0) === BOM_CODE ? stringRow[0].substring(1) : stringRow[0];
          }
